...
interface AgentSettingsTabProps {
  ...
  onRunHeartbeat: () => void;
  onRestart: () => void;
  onAgentUpdate?: () => Promise<void>; // Callback to refresh agent data
}
...
export default function AgentSettingsTab({
  ...
  // Generate integrations list from configuration
  const integrations = useMemo<Integration[]>(() => {
    const gmailIntegration = agentConfig.integrations?.gmail;
    const linkedInIntegration = agentConfig.integrations?.linkedin;
    ...
    return [
      {
        ...
        connected: gmailIntegration?.enabled || false,
      },
      {
        id: 'linkedin',
        name: 'LinkedIn',
        ...
        connected: linkedInIntegration?.enabled || false,
        comingSoon: true,
      },
    ];
  }, [agentConfig]);

  // Handle Gmail integration save
  const handleSaveGmail = useCallback(async (email: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Connected Gmail account: ${email}`,
      });
  ...
  // Handle LinkedIn integration save
  const handleSaveLinkedIn = useCallback(async (username: string) => {
    setIsSavingIntegration(true);
    try {
      const updatedConfig: AgentConfiguration = {
        ...agentConfig,
      ...
      toast({
        ...
        description: `Connected LinkedIn account: ${username}`,
      });
  ...
  // Handle integration connect button click
  const handleIntegrationConnect = useCallback((integrationId: string) => {
    ...
    if (integration.connected) {
      ...
      setDisconnectTarget(integrationId as 'gmail' | 'linkedin');
  ...
  const handleDisconnectIntegration = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${disconnectTarget === 'gmail' ? 'Gmail' : 'LinkedIn'} has been disconnected`,
      });
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <GmailIntegrationModal
        ...
        currentEmail={agentConfig.integrations?.gmail?.email}
      />
      ...
      <LinkedInIntegrationModal
        ...
        currentUsername={agentConfig.integrations?.linkedin?.username}
      />
